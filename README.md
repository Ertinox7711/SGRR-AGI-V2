...
<img src="assets/banner.svg" alt="SGRR AGI V2" width="100%" />

# SGRR AGI V2

**Le scaffold qui transforme Claude Code en agent autonome et proactif.**

Config, philosophie, hooks, mémoire et manifeste de plugins/skills — tout ce qu'il
faut pour qu'une install fraîche de Claude Code se comporte comme un agent de niveau « AGI ».
...
> que moi** (capacité 1:1), sans aucun de mes secrets ni de mes projets privés.
...
3. Claude ajoute les marketplaces, active les plugins, écrit `settings.json` et
   `CLAUDE.md`, crée la mémoire et les rules. **Fini.**
...
| **[`settings.template.json`](settings.template.json)** | `settings.json` prêt (Windows) : filet de permissions (`allow`/`ask`/`deny`), sous-agents pas chers (Sonnet), hooks d'injection de contexte. **Zéro secret.** |
| **[`settings.template.unix.json`](settings.template.unix.json)** | Même chose, hooks `sh` pour macOS/Linux. |
| **[`CLAUDE.md`](CLAUDE.md)** | La philosophie « AGI proactif » — dépersonnalisée. |
...
<div align="center">
<img src="assets/architecture.svg" alt="Architecture SGRR AGI V2" width="92%" />
</div>

Le patron canonique : **Commande → Agent → Skill**.
...
Le tout posé sur 6 fondations : `settings.json` (permissions), **hooks** (injection de
contexte par tour), **mémoire** (fichiers persistants), **MCP** (docs live, navigateur),
**plugins**, **rules** (contexte paresseux). Détail complet → **[`FONCTIONNEMENT.md`](FONCTIONNEMENT.md)**.
...
- Marketplace officielle des plugins Claude Code + auteurs communautaires (superpowers, caveman, gstack, anthropic-skills…) — voir [`SETUP.md`](SETUP.md).

Sous licence **MIT** — voir [`LICENSE`](LICENSE).

<div align="center">
<sub>Construit avec Claude Code · <code>SGRR AGI V2</code></sub>
</div>
