...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=_os.environ.get("VIDEO_USE_BACKEND", "scribe"),
    )
    ap.add_argument(
        "--whisper-model",
        default=_os.environ.get("WHISPER_MODEL", "base"),
    )
    args = ap.parse_args()
    ...
    if args.backend == "whisper" and args.workers > 1:
        print(f"note: forcing workers=1 for whisper backend (CPU-bound)")
        args.workers = 1
