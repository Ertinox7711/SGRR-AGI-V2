...
- Transcription backend resolves: either `ELEVENLABS_API_KEY` (Scribe API) OR `VIDEO_USE_BACKEND=whisper` (local faster-whisper). Set in `.env` at repo root. Whisper model via `WHISPER_MODEL=base|small|medium|large-v3`. Whisper has no diarization — `speaker_id` always `speaker_0`.
